/* -*- Mode:C++; c-file-style:"gnu"; indent-tabs-mode:nil; -*- */
 ...
 {
   uint32_t size = 8;//-----------------------------previous value 4------------------------------------------------------------created
   ...
   return (size + destsSize + 4 )  ;// destsSize added to size
 }
...
//-------------------------------------------------------s------------------------------------------
   ...
   //writing dest size
    i.WriteHtonU32(this->destsSize);
   //writing dest tuples
    for (std::vector<DestMessage>::const_iterator iter = this->destMessages.begin ();
        iter != this->destMessages.end (); iter++)
    {
      const DestMessage &dm = *iter;
      i.WriteHtonU32(dm.destAddr_hello.Get());
      i.WriteU16(dm.x_hello);
      i.WriteU16(dm.y_hello);
    }
//-------------------------------------------------------e------------------------------------------
 ...
 MessageHeader::Hello::Deserialize (Buffer::Iterator start, uint32_t messageSize)
 {
   ...
   NS_ASSERT (messageSize >= 8);//---------------------previous value 4---------------------------------------
 
   this->linkMessages.clear ();
   this->destMessages.clear ();//------------added
...
//-----------------------------------------------------------------------s------------------------------------created
   this->x = i.ReadU16();
   this->y = i.ReadU16();
   this->destsSize = i.ReadNtohU32 ();
//-----------------------------------------------------------------------e------------------------------------
 
   helloSizeLeft -= 12;//------------------------------------------previous value 4----------------------------
//-----------------------------------------------------------------------s-------------------------------------
   //deserializing dest tuples
   uint32_t destsSize1 =  destsSize;
   while (destsSize1)
     {
       DestMessage dm;
       dm.destAddr_hello = Ipv4Address (i.ReadNtohU32 ());
       dm.x_hello = i.ReadU16 ();
       dm.y_hello = i.ReadU16 ();
       this->destMessages.push_back (dm);
       destsSize1 -= 8;
     }
   helloSizeLeft -= destsSize;
...
//-----------------------------------------------------------------------e-------------------------------------
   ...
   while (helloSizeLeft)
     {
       LinkMessage lm;
       NS_ASSERT (helloSizeLeft >= 4);//--------------------------as same as previous value------------------
 ...
 { /*if(this->information_tc.size()>0){//to show tc size.
   std::cout<<"tc size = -------------------------------------------------"<<this->information_tc.size()*16<<std::endl;}*/
   return 6 + this->neighborAddresses.size () * IPV4_ADDRESS_SIZE + this->information_tc.size()*16 ;//this->information_tc.size()*16 added and previous value was 4 instead of 6.
 }
 ...
 MessageHeader::Tc::Serialize (Buffer::Iterator start) const
 {
   ...
   i.WriteU16 (this->information_tc_size);//---------------------added
...
//----------------------------------------s----------------------------------------------------------
   for (std::vector<info_tc>::const_iterator iter = this->information_tc.begin ();
        iter != this->information_tc.end (); iter++)
    {
      const info_tc &itc = *iter;
      i.WriteHtonU32(itc.tcm_mpr.Get());
      i.WriteHtonU32(itc.tcm_mprSelector.Get());
      i.WriteHtonU32(itc.tcm_dest.Get());
      i.WriteU16(itc.tcm_cost);
      i.WriteU16(itc.tcm_chCost);
    }
//----------------------------------------e----------------------------------------------------------
 }
 ...
 MessageHeader::Tc::Deserialize (Buffer::Iterator start, uint32_t messageSize)
 {
   ...
   NS_ASSERT (messageSize >= 6);//previous 4
 
   this->ansn = i.ReadNtohU16 ();
   i.ReadNtohU16 (); // Reserved
   this->information_tc_size = i.ReadU16();
 
   NS_ASSERT ((messageSize - 6 - information_tc_size ) % IPV4_ADDRESS_SIZE == 0);//previous 4 and -information_tc_size added
   int numAddresses = (messageSize - 6 - information_tc_size ) / IPV4_ADDRESS_SIZE;//previous 4 and -information_tc_size added
   this->neighborAddresses.clear ();
   for (int n = 0; n < numAddresses; ++n)
     {
...
//---------------------------------------------s-------------------------------------------------------
   /*if(information_tc_size)
     {
      std::cout<<"  ----------   "<<remaining_size<<std::endl;//--------------added
     }*/
   while (information_tc_size)
     {
       info_tc imc;
       imc.tcm_mpr= Ipv4Address (i.ReadNtohU32 ());
       imc.tcm_mprSelector= Ipv4Address (i.ReadNtohU32 ());
       imc.tcm_dest= Ipv4Address (i.ReadNtohU32 ());
       imc.tcm_cost = i.ReadU16 ();
       imc.tcm_chCost = i.ReadU16 ();
       this->information_tc.push_back (imc);
       information_tc_size -= 16;
     }
    /*if(this->information_tc.size()>0){    //to show tc size.-----------------added
      std::cout<<"tc size from deserialize= -------------------------------------------------"<<this->information_tc.size()*16<<std::endl;}*/
 //---------------------------------------------e------------------------------------------------------
   return messageSize;
 }
 ...
 //-------------------------------------------------------------s--------------------------------------------------
 // ---------------- OLSR TC modified Message -------------------------------
 /*
 uint32_t
 MessageHeader::Tc::GetSerializedSize (void) const
 {
    return this->tcm_size;
 }
