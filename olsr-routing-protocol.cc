...
//-------------------------------------------------------------s--------------------------------------------------------------------
 void
 RoutingProtocol::AddDestTuple (const DestTuple &tuple)
 {
   m_state.InsertDestTuple (tuple);
   //IncrementAnsn ();
 }

 void
 RoutingProtocol::ShowDests ()
 {
   m_state.ShowDest();
 }

 void 
 RoutingProtocol::PopulateDestSet (const olsr::MessageHeader &msg,
                            const olsr::MessageHeader::Hello &hello)
  {

   Ipv4Address a = msg.GetOriginatorAddress();
   if( a=="10.1.1.2"||a=="10.1.1.3"||a=="10.1.1.4"||a=="10.1.1.1"||a=="10.1.1.5")  
    {
       DestTuple d_tuple;
       d_tuple.destAddr = msg.GetOriginatorAddress();
       d_tuple.dtlocX = hello.GetNodeLocX();
       d_tuple.dtlocY = hello.GetNodeLocY();
      // std::cout<<m_mainAddress<<"  dest= "<<d_tuple.destAddr<< "  "<<d_tuple.dtlocX<<"   "<<d_tuple.dtlocY<<std::endl;
       AddDestTuple (d_tuple);
    }

   //show dest receive and update dest Table according to hello msg.
    for (std::vector<olsr::MessageHeader::Hello::DestMessage>::const_iterator it = hello.destMessages.begin (); it != hello.destMessages.end (); it++)
    {
       DestTuple dt;
       dt.destAddr = it->destAddr_hello;  
       dt.dtlocX = it->x_hello;  
       dt.dtlocY = it->y_hello;  
       //std::cout<<"Dest Address: "<<dt.destAddr<<", x= "<<dt.dtlocX<<", y= "<<dt.dtlocX<<std::endl;
       AddDestTuple (dt);
    }
 ...
 void
 RoutingProtocol::PopulateMpr_SetOfSrc()
 { 
  if(m_state.GetDestSize() != 0) 
   {
   const DestSet &dests1 = m_state.GetDests();
   for (DestSet::const_iterator iter = dests1.begin (); iter != dests1.end (); iter++)
    {
       Mpr_Tuple mpr_temp;
       mpr_temp.mt_selector = "0.0.0.0";//we can not assign this value as NULL. if so then showing SIG error.
       mpr_temp.mt_dest = iter->destAddr;
       mpr_temp.mt_cost = 0;
       mpr_temp.mt_ch_cost = 0;
       mpr_temp.mt_locationX = iter->dtlocX;
       mpr_temp.mt_locationY = iter->dtlocY;//std::cout<<"Dest Address: "<<mpr_temp.mt_dest<<std::endl;
       m_state.InsertMpr_Tuple (mpr_temp); 
    }
   }
 }

 void 
 RoutingProtocol::PopulateMpr_Set (const olsr::MessageHeader &msg,
                            const olsr::MessageHeader::Tc &tc)
 { 
  if(tc.information_tc.size() > 0) 
   {
   const DestSet &dests2 = m_state.GetDests();
   for (std::vector<olsr::MessageHeader::Tc::info_tc>::const_iterator it = tc.information_tc.begin (); it != tc.information_tc.end (); it++)
     {
       if(it->tcm_mpr == m_mainAddress)
         {
          Mpr_Tuple mpr_temp1;
          mpr_temp1.mt_selector = it->tcm_mprSelector;
          mpr_temp1.mt_dest = it->tcm_dest;
          mpr_temp1.mt_cost = it->tcm_cost;
          mpr_temp1.mt_ch_cost = it->tcm_chCost;
         
          for (DestSet::const_iterator iter = dests2.begin (); iter != dests2.end (); iter++)
            {
              if(iter->destAddr == mpr_temp1.mt_dest)
                {
                   mpr_temp1.mt_locationX = iter->dtlocX;
                   mpr_temp1.mt_locationY = iter->dtlocY;
                   break;
                }    
            }
       
          m_state.InsertMpr_Tuple (mpr_temp1); 
          break;
         }
     }
   }
...
//----------------------------------------------------------------------------------s-----------------------created
  //std::cout<<"from node  "<< senderIface<< ", after receiving location x= "<<hello.GetNodeLocX()<<", y= "<<hello.GetNodeLocY()<<std::endl;
   PopulateDestSet (msg, hello);
  //showing destination tuples.
   //std::cout<<"-------Dests of "<< m_mainAddress<<"=---------------\n";ShowDests();std::cout<<"------------------------------------------\n";
  
  //updating mpr table of source node.
  if (m_mainAddress == "10.1.1.7")
    {
      PopulateMpr_SetOfSrc();
      //std::cout<<"mpr table of"<<m_mainAddress<<": \n";//showing mpr table of source.
      //m_state.ShowMpr_Set();std::cout<<"\n";
      //std::cout<<"mpr size of: "<<m_mainAddress<<" is: "<<m_state.GetMpr_Size();std::cout<<"\n";//showing size of mpr table of source.
      
      m_state.aStar();//run A* and updating cost tuple.
      //std::cout<<"Cost table of"<<m_mainAddress<<": \n";//showing cost table of source.
      //m_state.ShowCostSet();std::cout<<"\n";
      //std::cout<<"Cost size of: "<<m_mainAddress<<" is: "<<m_state.GetCostSize();std::cout<<"\n";//showing size of cost table of source.
    }
   //count mpr node.
     //if( m_state.GetMpr_Size() > 0 )
      // {
       // std::cout<<"MPR nodes: "<<m_mainAddress<<": \n";   
      // }
   //Showing Cost Table of all nodes.
      //std::cout<<"Cost table of"<<m_mainAddress<<": \n";//showing cost table of all nodes.
      //m_state.ShowCostSet();std::cout<<"\n";
//----------------------------------------------------------------------------------e------------------------------
...
//--------------------------------------------------s--------------------------------------------------------
   //if(senderIface == "10.1.1.1"){std::cout<<"From: "<<senderIface<<", received size(byte): "<<tc.information_tc.size()*16<<std::endl;}
   //std::cout<< "From: "<<senderIface<<std::ednl;
   for (std::vector<olsr::MessageHeader::Tc::info_tc>::const_iterator it_tc = tc.information_tc.begin (); it_tc != tc.information_tc.end (); it_tc++)
      { 
         if(it_tc->tcm_mpr == m_mainAddress)
           { 
            PopulateMpr_Set(msg,tc);
            m_state.aStar();
            break;
           }
...
//--------------------------------------------s-----------------------------------------
// send location of sender node.
  Ptr<MobilityModel>mob = m_ipv4->GetObject<MobilityModel>();
  uint16_t x = (int)mob->GetPosition().x;
  uint16_t y = (int)mob->GetPosition().y;
  //showing location before sending 
  //std::cout<<"from node  "<< m_mainAddress<< ", before sending location x= "<<x<<", y= "<<y<<std::endl;

  hello.SetNodeLoc(x,y);//set sender loc 
 //printing hello size
 // uint32_t helloSize = 9;// ---------------------hello size 1/4-------------------
 //sending dest tuples via hello
  std::vector<olsr::MessageHeader::Hello::DestMessage> &destMessages = hello.destMessages;
  const DestSet &dests = m_state.GetDests();

  for (DestSet::const_iterator iter = dests.begin (); iter != dests.end (); iter++)
  {
    olsr::MessageHeader::Hello::DestMessage destinationMessage ;
    destinationMessage.destAddr_hello = iter->destAddr;
    destinationMessage.x_hello = iter->dtlocX;
    destinationMessage.y_hello = iter->dtlocY;
    destMessages.push_back(destinationMessage);
  }
 // helloSize += destMessages.size() * 4;//-----------------hello size 2/4 ------------------------------
  hello.destsSize = m_state.GetDestSize();//sending dest table size
  //std::cout<<m_state.GetDestSize()<<std::endl;
...
//--------------------------------------------e-----------------------------------------
   ...
   for (LinkSet::const_iterator link_tuple = links.begin ();
        link_tuple != links.end (); link_tuple++)
     {
       ...
       linkMessage.neighborInterfaceAddresses.push_back
         (link_tuple->neighborIfaceAddr);
    //std::cout<<"from hello "<< link_tuple->neighborIfaceAddr << std::endl;//--------------------------created
      ...
      // helloSize += 5; //------------------------------hello size 3/4---------------------------------------
     }
   //std::cout<<helloSize<<std::endl;//------------------hello size 4/4
 ...
 {  //std::cout<<"Generating TC from"<<m_mainAddress<<std::endl;//-----------------------------------------tc created
   NS_LOG_FUNCTION (this);
   ...
   tc.ansn = m_ansn;
   tc.information_tc_size = m_state.GetMpr_Size();//-------------------added
...
//-----------------------------------------------------------s-------------------------------------------
 //sending mpr information through tc msg.
  std::vector<olsr::MessageHeader::Tc::info_tc> &information_tc = tc.information_tc;
  const Mpr_Set &mpr_set1 = m_state.GetMpr_Set();
  const CostSet &costSet1 = m_state.GetCostSet ();
  const NeighborSet &neighbors1 = m_state.GetNeighbors ();
  if( m_state.GetMpr_Size() > 0 )//--------extra--> m_state.GetMprSelectors ().size () > 0---
  {std::cout<< m_mainAddress<<std::endl;//-----------------------------------------show mpr
    //size of tc msg
      //uint32_t sizeTc = 0;//-----sizeTc
   for (Mpr_Set::const_iterator iter1 = mpr_set1.begin ();
        iter1 != mpr_set1.end (); iter1++)
     { 
      // sizeTc = sizeTc + 16;//-----sizeTc
       olsr::MessageHeader::Tc::info_tc info;
       info.tcm_mprSelector = m_mainAddress;//inserting mpr selector of tc tuple.
       info.tcm_dest = iter1-> mt_dest;//inserting destination of tc tuple.
       for (CostSet::const_iterator iter2 = costSet1.begin ();//inserting mpr and cost of tc tuple.
            iter2 != costSet1.end (); iter2++)
            {
              if(iter1 -> mt_dest == iter2-> ct_dest)
               {
                 info.tcm_mpr = iter2-> ct_nextMpr;
                 info.tcm_cost = iter2-> ct_cost;
                 for (NeighborSet::const_iterator iter3 = neighbors1.begin ();//inserting ch cost of tc tuple.
                      iter3 != neighbors1.end (); iter3++)
                      {
                         if(iter2->ct_nextMpr == iter3-> neighborMainAddr)
                           {
                             info.tcm_chCost = iter1-> mt_ch_cost + iter3-> ch_cost;
                             break; 
                           }
                      }
                 break;
       ...
       information_tc.push_back (info);//sending tc message information
     }
  //print tc size
 // std::cout<< sizeTc << std::endl;//----sizeTc
  }
 
// std:: cout<<"From: "<<m_mainAddress<< ", MPR size(row number) = "<< m_state.GetMpr_Size()/16 <<", tc size= "<<information_tc.size() <<", neigh size = "<<neighbors1.size()<<", Cost size = "<<costSet1.size()<<std::endl;
...
//-------------------------------------------------------s-------------------------------------------
/* void
 RoutingProtocol::SendTc () //modified tc
 { std::cout<<"from TC from"<<m_mainAddress<<std::endl;//-----------------------------------------tc created
   NS_LOG_FUNCTION (this);
...
//-------------------------------------------------------e-------------------------------------------
 ...
 RoutingProtocol::SendHna ()
 {
   ...
   //hna size
  // uint32_t sizeHna = 0;//------------------------------------size hna 1/3-----------------
   ...
   for (Associations::const_iterator it = localHnaAssociations.begin ();
     ...
     { 
       ...
       //sizeHna += 8;//---------------------size hna 2/3-----------------------------
     }
  // std::cout<<sizeHna<<std::endl;//---------size hna 3/3----------------------------
...
//-----------------------------------------------------------------------------s-----------------------------created
     float wi;
      if(hello.willingness == OLSR_WILL_ALWAYS)
       {//std::cout<<"willingness= "<<"7"<<std::endl;
         wi = 7;
       }
     else if (hello.willingness == OLSR_WILL_NEVER)
       {//std::cout<<"willingness= "<<"0"<<std::endl;
         wi = 1;
       }
     else if (hello.willingness == OLSR_WILL_LOW)
       {//std::cout<<"willingness= "<<"1"<<std::endl;
         wi = 2;
       }
     else if (hello.willingness == OLSR_WILL_HIGH)
       {//std::cout<<"willingness= "<<"6"<<std::endl;
         wi = 6;
       }
     else //(hello.willingness == OLSR_WILL_DEFAULT)
       {//std::cout<<"willingness= "<<"3"<<std::endl;
         wi = 3;
       }

       nb_tuple->locX = hello.GetNodeLocX();//getting sender location and insert neigbor tuple
       nb_tuple->locY = hello.GetNodeLocY();
 
       Ptr<MobilityModel>mob = m_ipv4->GetObject<MobilityModel>();//receiving neighbor location
       m_state.m_LocX = (int)mob->GetPosition().x;
       m_state.m_LocY = (int)mob->GetPosition().y;
       
       float s_dis = sqrt(pow((nb_tuple->locX - m_state.m_LocX),2.0)+pow((nb_tuple->locY - m_state.m_LocY),2.0));//calculating straight dis
       
       nb_tuple->ch_cost = uint16_t(m_state.Weight_cal(s_dis,wi));//assigning channel cost

  //showing location after receiving      
  // std::cout<<"from node  "<< msg.GetOriginatorAddress () << ", after receiving location x= "<<nb_tuple->locX<<", y= "<<nb_tuple->locY;
  //std::cout<<" dist= "<<s_dis<<" cost= "<<nb_tuple->ch_cost <<" receiver= "<<m_mainAddress<<std::endl;
//-----------------------------------------------------------------------------d------------------------------------
     }
 }
 ...
 //previous TcTimerExprire

 void
 RoutingProtocol::TcTimerExpire ()
 {

   if (m_state.GetMprSelectors ().size () > 0) //m_state.GetMpr_Size() > 0 ---->added---original-->m_state.GetMprSelectors ().size () > 0
     {
        /*if(m_state.GetMpr_Size() > 0)
          {
                 std::cout<<m_state.GetMpr_Size() <<std::endl;
          }//-----added*/
       SendTc ();
     }
   else
